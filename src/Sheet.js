  ...
  render() {
    ...
    const guides = sheet.map(
      ...
      <text x={board.x+board.w*.5} y={board.y+board.h*.5} textAnchor="middle" fontSize={.06*board.h} fill={guide.fill}>{board.label + 
        (
          (board.id === "middle" && dimensions.leftShelfHeight2 && dimensions.dadoDepth > 0) ? 
            " (Left side)" : 
            (
              ( board.id === "middle" && dimensions.rightShelfHeight2 && dimensions.dadoDepth > 0) ? 
                " (Right side)" : 
                ""
            )
        )}</text>
      <text x={board.x+board.w*.5} y={board.y+board.h*.6} textAnchor="middle" fontSize={.06*board.h} fill={guide.fill}>{board.w + units + " x " + board.h + units}</text>
      </g>)
      }
    );
    let dados = [];
